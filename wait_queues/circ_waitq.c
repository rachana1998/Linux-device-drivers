...
static wait_queue_head_t _waitq;   //the process will waits 
...
static ssize_t circ_read(struct file *filep, char __user *ubuff , size_t cnt, loff_t *offset)
{
        int i;
       	int ret,mini;
        wait_event_interruptible(_waitq,CIRC_CNT(cbuff.head,cbuff.tail,SIZE)>0);
...
static int __init circ_init(void)
{
        int ret;
        devnum = MKDEV(55,0);
        ret = register_chrdev_region(devnum,1,"mywaitqdevice");
        if(ret)
        {
