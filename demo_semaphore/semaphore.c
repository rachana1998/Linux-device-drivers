...
#include<linux/delay.h> //for dealy 
...
{
        int ret,i;
	down_interruptible(&_devp->_sem);  //down ie., post or lock the semaphore 
...
{	
	...
	sema_init(&_dev[i]._sem,1); //semaphore initialsation  it allow only one process to execute in critical section
